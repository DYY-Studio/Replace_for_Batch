...
for /f "tokens=*" %%c in ('set /a %input_len%-%delims_len%') do (
	for /l %%a in (1,1,%%~c) do (
...
set "work_string=%input_string%"
set "work_string2="
for /l %%a in (1,1,%delims_len%) do (
	call :RB.add_end
)
set "output_string=%output_string%%work_string2%"
goto RB.replace_finish

:RB.add_end
set "work_string2=%work_string:~-1%%work_string2%"
set "work_string=%work_string:~0,-1%"
goto :EOF
:RB.list_input_get
